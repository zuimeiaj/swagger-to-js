class SwaggerJS {
  ...
  getRequestBody(item, comments) {
    let defPath = this._getObjectByPath(item, ['requestBody', 'content', 'application/json', 'schema', '$ref']);
  ...
  getResponseBody(item, comments) {
    if (item.operationId == 'getPlans') {
      console.log(item);
    }
    let defPath = this._getObjectByPath(item, ['responses', '200', 'content', '*/*', 'schema', '$ref']);
    if (!defPath) return;
    comments.push(` * @returns {${defPath.split('/').pop()}} `);
  }
  getComment(item) {
    const comments = [];
    comments.push('/**');
